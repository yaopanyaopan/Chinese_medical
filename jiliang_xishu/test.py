...
import data.limit_model.Model_limit
...
def loadData(preName):
   #处理好的数据集 已经正负例平衡
    ...
    train_y = []
    sumZeroList=[]
    presCsvname = 'data/fangji/'+preName+'/'+preName+'_data2.csv'        # 只有药物做了归一化
...
if __name__ == '__main__':
    ...
    # ----------------------------------------
        ...
        for row in range(len(data_x)):
            row_data = []
            for col in range(0,len(data_x[row]),2):
                row_data.append(data_x[row][col])
            # print(len(row_data))
            for col in range(1,len(data_x[row]),2):
                row_data.append(data_x[row][col])
        ...
        for i in range(4,5):
            ...
            for k in range(1,2):
                for j in range(1, 2):
                    ...
                    print('开始训练:')
                    model = data.limit_model.Model_limit.Model()
                    tf.reset_default_graph()
                    with tf.Graph().as_default():
                        with  tf.Session() as sess:
                            model.build_graph(int(dimension),lamda )
                            sess.run(tf.global_variables_initializer())
                            for step in range(maxiter):
                                loss_all = []
                                for j in range(len(X_train)):
                                    result = sess.run([model.train, model.theta_new, model.loss],
                                                      feed_dict={model.x_train: X_train[j], model.y_train: [y[j]]})
                                    loss_all.append(result[2])
                                print(step,'---loss:', float(sum(loss_all))/len(loss_all))
                    ...
                    print ('由LASSO约束后变为0的参数个数：', count_zero )
                    print ("step 3: testing...")
                    # accuracy = model.score(X_train , y)   # 返回　平均准确率
                    ...
                    for item in range(0,int(dimension)):
                        # print(weightdata[item],weightdata[item])
                        zz = []
                        zz.append(num1)
                        zz.append(float(weightdata[item][0]) + float(weightdata[item+int(dimension)][0]))
