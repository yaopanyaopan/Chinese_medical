...
def loadData(preName):
   #处理好的数据集 已经正负例平衡
    ...
    presCsvname = 'data/fangji_zscore/'+preName+'/'+preName+'_data2*.csv'  # 只有药物做了归一化
    funcCsvname = 'data/fangji_zscore/'+preName+'/'+'FuncFeature_'+preName+'.csv'
        ...
        for i in range(4,5):
            ...
            for k in range(1,2):
                ...
                # 使用　坐标轴下降法(可同时　优化Ｌ１，Ｌ２正则化)
                    ...
                    weightdata = coef    # 对应参数
                    csvname = 'data/fangji_zscore/'+preName+'/'+preName+'_count_medical*.csv'
                    fr3 = open(csvname,'r')
                    ...
                    #         num1 += 1
                    for item in range(0,len(weightdata),2):
                            # print(weightdata[item],weightdata[item])
                            zz = []
                            zz.append(num1)
                            zz.append(float(weightdata[item])+float(weightdata[item+1]))
                            weightlist.append(zz)
                            num1 += 1
