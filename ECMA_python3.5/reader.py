...
def load_csv(filepath, target_columns=-1, columns_to_ignore=None,
    has_header=True, n_classes=None, target_dict=None, mode_transform=False,usePreVector=False):
  ...
  with gfile.Open(filepath) as csv_file:
    ...
    for i, d in enumerate(data_file):
      #show data,_d.strip()创建数据时每个句子后面多了一个空格
      # for _i, _d in enumerate(d):
      #   print(_i, _d.strip())
      data.append([_d.strip() for _i, _d in enumerate(d) if _i not in target_columns and _i not in columns_to_ignore])
